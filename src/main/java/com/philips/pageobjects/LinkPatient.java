...
public class LinkPatient {
	...
	By link = By.id("link");
	By successMessage = By.id("successMessage");// check the sucess message

	public LinkPatient(WebDriver driver) {
		this.driver = driver;
	...
	public void setSourceExtensonId(String extentionId) {
		try {
			Log.info("setting source aggregateId");
			driver.findElement(sourceExtensionId).clear();
			driver.findElement(sourceExtensionId).sendKeys(extentionId);
		} catch (Exception e) {
			Log.error("Error in setting source aggregate id:" + e.getMessage());
		}
	}

	public void setTargetExtensonId(String extentionId) {
		try {
			Log.info("setting target aggregateId");
			driver.findElement(targetExtensionId).clear();
			driver.findElement(targetExtensionId).sendKeys(extentionId);
		} catch (Exception e) {
			Log.error("Error in setting target aggregate id:" + e.getMessage());
		}
	}

	public void searchFragments() {
		try {
			Log.info("Searching for the patient fragments");
			driver.findElement(search).click();
		} catch (Exception e) {
			Log.error("Error in searching fragments:" + e.getMessage());
		}
	}
	...
	public void linkFragment() {
		try {
			...
			//Log.info(driver.findElement(successMessage).getText());
			 Log.info("Sucessfully linked the fragemnts");
		} catch (Exception e) {
			System.out.println("Unable to linke th fragment:");
			Log.error("Unable to link the fragment:" + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public Map<String, List<String>> getTargetAggregateDemographic() {
		...
		try {
			int fragmentsNumber = driver
					.findElements(
							By.xpath("//table[@id='targetFragmentdataTable']/tbody/tr"))
					.size();
			// fragmentsNumber=fragmentsNumber-1;
			String mainwindow = driver.getWindowHandle();
			System.out.println("Main Window:" + mainwindow);
			driver.findElement(
					By.xpath("//*[@id='targetFragmentdataTable']/tbody/tr["
							+ fragmentsNumber + "]/td[6]")).click();
			for (String winHandle : driver.getWindowHandles()) {
				driver.switchTo().window(winHandle);
				System.out.println(driver.getTitle());
			}
			(wait.until(ExpectedConditions.elementToBeClickable(driver
					.findElement(By.id("ui-id-2"))))).click();// Click the Basic
																// Details
																// Page-before
																// capturing
																// values
			List<WebElement> fnamelist = driver.findElements(By
					.xpath("//ul[@id='firstname']/li"));
			...
			List<WebElement> lnamelist = driver.findElements(By
					.xpath("//ul[@id='lastname']/li"));
			...
			gendertemp = driver.findElement(By.xpath("//span[@id='gender']"))
					.getText();
			...
			dob.add(driver.findElement(By.xpath("//span[@id='dob']")).getText()
					.replaceAll("-", ""));
			rootid.add(driver.findElement(By.xpath("//span[@id='root']"))
					.getText());
			aggregateId.add(driver.findElement(
					By.xpath("//span[@id='extension']")).getText());
			mmn.add(driver.findElement(
					By.xpath("//span[@id='mothersmaidenname']")).getText());
			...
			List<WebElement> Scopingorgtablelist = driver.findElements(By
					.xpath("//table[@id='oidTable']/tbody/tr"));
			...
			if (!Scopingorgtablelist.isEmpty()) {
				for (WebElement d : Scopingorgtablelist) {
					...
					if (columns.get(0).getText()
							.equals("No data available in table")) {
						break;
					} else {
						for (int i = 0; i <= columns.size(); i += 3) {
							scopingOid.add(columns.get(i).getText());
							AssigningAuthorityName.add(columns.get(i + 1)
									.getText());
							AssigningAuthorityExtension.add(columns.get(i + 2)
									.getText());
						}
					}
				}
			...
			driver.findElement(
					By.xpath("//button[@type='button']/span[@class='ui-button-icon-primary ui-icon ui-icon-closethick']"))
					.click();
			...
			targetPatientDetailsMap.put("assigningauthorityname",
					AssigningAuthorityName);
			targetPatientDetailsMap.put("assigningauthorityextension",
					AssigningAuthorityExtension);
			driver.switchTo().window(mainwindow);

		} catch (Exception e) {
			System.out
					.println("Exception in getting target Aggregate demographics");
			e.printStackTrace();
		}
	...
	public Map<String, List<String>> getSourceAggregateDemographic() {
		...
		try {
			...
			/*
			 * Get all the demographic details from the Basic Details
			 * tab-->Start
			 */
			List<WebElement> fnamelist = driver.findElements(By
					.xpath("//ul[@id='firstname']/li"));
			...
			sourcePatientDetailsMap.put("assigningauthorityname",
					AssigningAuthorityName);
			sourcePatientDetailsMap.put("assigningauthorityextension",
					AssigningAuthorityExtension);
			...
			System.out
					.println("Exception in getting source Aggregate demographics");
			e.printStackTrace();
		}
	...
	public void selectFragmentToLink(String root, String extention) {
		...
		try {
			...
			Log.info("No of source Fragment Rows:" + fragmentsNumber);
			for (int i = 1; i < fragmentsNumber; i++) {
				...
				if (fragementRoot.equalsIgnoreCase(root)
					...
					Log.info("FragmentRoot:" + fragementRoot
							+ " Fragment Extention:" + fragmentExtn
							+ " Will be selected for link");
					driver.findElement(
							By.xpath("//*[@id='dataTable']/tbody/tr[" + i
			...
			if (!flag) {
				...
				Log.error("Fragement Does not exist to select");
				Assert.assertFalse(true);
			}

		} catch (Exception e) {
			System.out.println("Exception Selecting the fragment:");
			Log.error("Exception Selecting the fragment:");
			e.printStackTrace();
		}
